        ...
        home: StatefulBuilder(
          builder: (context, setState) {
            ...
            final tabs = List.generate(
              4,
              (i) => GlassTab(label: 'Tab $i', icon: const Icon(Icons.circle)),
            );
                ...
                child: SizedBox(
                  ...
                  child: searchable
                      ? GlassTabBar.searchable(
                          ...
                          quality: quality,
                          indicatorBorderRadius: indicatorRadius,
...
void _verifyGeometry(WidgetTester tester) {
  ...
  final masks = find.byWidgetPredicate(
    (w) => w is ClipPath && w.clipper is JellyClipper,
  );
  ...
  for (final element in find.byType(AnimatedGlassIndicator).evaluate()) {
    ...
    final path = LiquidRoundedRectangle(
...
void main() {
  for (final searchable in [false, true]) {
    testWidgets(
        '1.7.2 custom radius and background quality searchable=$searchable',
        (tester) async {
      final harness =
          _Harness(quality: GlassQuality.premium, searchable: searchable)
            ..indicatorRadius = 12
      ...
      final backgrounds = tester.widgetList<RepaintBoundary>(
      ...
      for (final background in backgrounds) {
        expect(
            (background.child! as AdaptiveGlass).quality, GlassQuality.minimal);
      }
      await tester.pumpAndSettle();
    });
    for (final quality in GlassQuality.values) {
      for (final direction in TextDirection.values) {
        testWidgets(
          ...
          'through travel, reverse and rest',
          (tester) async {
            ...
            for (var i = 0; i < 12; i++) {
              ...
              _verifyGeometry(tester);
              expect(
                identical(tester.widget(backgroundFinder), background),
                isTrue,
    ...
    for (final platformView in [false, true]) {
      for (final masking in MaskingQuality.values) {
        testWidgets(
          ...
          final harness =
              _Harness(quality: GlassQuality.standard, searchable: searchable)
                ..platformView = platformView
                ..masking = masking;
          await tester.pumpWidget(harness.build());
          await tester.pumpAndSettle();
          final bar = find.byType(
            searchable ? SearchableTabIndicator : TabIndicator,
          );
          ...
          for (final indicator in indicators) {
            expect(indicator.alignment.resolve(TextDirection.ltr).x, 1);
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        for (final indicator in tester.widgetList<AnimatedGlassIndicator>(
          ...
          expect(indicator.thickness, 0);
          expect(
            indicator.alignment.resolve(TextDirection.ltr).x,
